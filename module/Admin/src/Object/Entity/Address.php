 ...
 *      @ORM\Index(name="fk_address_address_type1_idx", columns={"address_type_id"}),
...
class Address extends Filtered
{
    ...
    public function getAddressTypeObj()
    {
        return $this->addressType->getAll();
    }
    ...
    public function getLocationTypeObj()
    {
    ...
    public function getStreetTypeObj()
    {
        return $this->streetType->getAll();
    }
    ...
    public function getAddressNameSimple(){
        $region = $this->getRegion();

        $first_sep = '';
        $first_sep = ($this->getRegion()&&$this->getLocation())?', ':null;
        $location = $this->getLocationType()->getShortName();
        $location .= $this->getLocation();

        $second_sep = '';
        $second_sep = ($this->getLocation() && $this->getStreet())?', ':null;
        $street = $this->getStreetType()->getShortName();
        $street .= $this->getStreet();

        $third_sep = ($this->getStreet()&&$this->getHouse())?', ':null;
        $house = $this->getHouse();
        $house .= $this->getHouseSuffix();

        return $region.$first_sep.$location.$second_sep.$street.$third_sep.$house;
    }
    ...
    public function getAll(){
        return array(
            ...
            'address_type' => $this->getAddressTypeObj(),
            'is_main' => $this->getIsMain(),
            'post_code' => $this->getPostCode(),
            'client' => $this->getClient(),
            'region' => $this->getRegion(),
            'area' => $this->getArea(),
            'location_type' => $this->getLocationTypeObj(),
            'location' => $this->getLocation(),
            'street_type' => $this->getStreetTypeObj(),
            'street' => $this->getStreet(),
            'house' => $this->getHouse(),
            'house_suffix' => $this->getHouseSuffix(),
            'building' => $this->getBuilding(),
            'flat' => $this->getFlat(),
            'post_box' => $this->getPostBox()
        );
    }
    ...
    public function getAddressSimple(){
        return array(
            'id' => $this->getId(),
            'address_simple' => $this->getAddressNameSimple(),
            'address_type_simple' => $this->getAddressType()->getName()
        );
    }
}
